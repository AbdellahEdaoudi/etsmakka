...
export default function ContactForm({ dict }) {
    ...
    const validate = (data) => {
        const newErrors = {}
        if (!data.name?.trim()) newErrors.name = dict.contact.form.error_required
        if (!data.subject?.trim()) newErrors.subject = dict.contact.form.error_required
        if (!data.message?.trim()) newErrors.message = dict.contact.form.error_required
    ...
    return (
        <form onSubmit={SendMessage} className="space-y-4" noValidate>
            ...
            <div className="space-y-2">
                <label className="text-sm font-medium text-foreground/80 px-1">{dict.contact.form.subject}</label>
                <input
                    maxLength={200}
                    type="text"
                    name="subject"
